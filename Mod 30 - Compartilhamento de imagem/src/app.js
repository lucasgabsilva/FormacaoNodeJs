...
let JWTsecret = "983476ty-pq9gayewpguhp349786typq34o-9"
...
app.post("/auth", async (req,res) => {
    ...
    let isPassRight = await bcrypt.compare(password,user.password)
    ...
    jwt.sign({email,user: user.name, id: user._id},JWTsecret,{expiresIn: '24h'},(err, token) => {
        if(err){
            res.sendStatus(500)
            console.log(err)
        }else{
